...
| **Price range filter bounds are inclusive** | The range slider and its start/end inputs always build an **inclusive** expression: the storefront filter component hardcodes both the lower- and upper-bound inclusion flags to true, matching the platform's bracket-inclusive filter syntax, and there is no exclusive-bound path in the storefront price-filter UI. An item priced exactly at either bound **is included**. Confirmed live by setting an upper bound at a value where items exist and seeing them returned. This row previously asserted the opposite (a boundary-priced item being excluded), which the implementation contradicts | Low | A boundary-priced item silently missing from results | [OBSERVED] |
| **Multi-select filter AND vs OR confusion** | Multiple values selected within the **same** facet combine as a **union (OR)** by design: the storefront's filter builder joins same-facet selections into one comma-separated field expression — the platform's documented OR syntax — and no code path repeats the field name to force an intersection. Confirmed live on a catalog facet, where adding further values returned exactly the sum of each value's own facet count for non-overlapping values; under intersection semantics the same selection would return zero. The regression this row guards: a same-facet multi-select returning fewer rows than its largest single selection, or zero | Low | No results, user confusion | [OBSERVED] |
...
**Amended:** 2026-08-27 (auto-applied, triangulated — ECL-AUDIT-2026-08-27). **Rows 1 and 4** changed. Row 1 named a mechanism the implementation contradicts: the sort code and the facet expression are separate route query parameters and the filter-apply path resets only the page, so a filter cannot clear the sort — rewritten to the risk that survives, a server-derived sort option list re-read per response. Row 4 was correct but mechanism-free, and the mechanism turns out to be an asymmetry worth stating: three control filters persist in browser storage (availability defaulting to on) while facet filters are URL-only. **Source:** the products composable's two independent query params, its filter-apply path, its three storage-backed controls, and the per-response sortings assignment; the storefront search query's sortings selection. **Live:** the facet sidebar, the sort dropdown, the three checkbox controls with availability pre-checked, and an active-filter chip row with a reset control, all on a keyword result page.

**Amended:** 2026-09-03 (auto-applied, triangulated — ECL-AUDIT-2026-09-03). **Rows 2 and 3** changed, on all three axes agreeing. Row 2 was **DRIFT stating the inverse of the behaviour** — it asserted that an item priced exactly at a range bound is excluded, while the storefront filter component hardcodes inclusion on both bounds and a live boundary query returns the boundary-priced items; the row now records the inclusive contract and keeps the exclusion as the regression it guards. Row 3 was the section's last `[THEORETICAL]` row and is promoted to `[OBSERVED]`: same-facet multi-select is a union, verified by paired live controls whose combined result count equalled the sum of the individual facet counts for non-overlapping values — an intersection would have returned zero. Rows 1 and 4 CONFIRMED unchanged, and row 1's outstanding live-axis caveat is closed above. **Docs:** the platform's indexed-search query-syntax reference states that multiple comma-separated values in one field match if *any* one matches (an OR), the multi-select faceting policy is documented as enabled by default with no extra configuration, and the same reference defines square-bracket ranges as inclusive of both bounds. **Source:** the storefront's facet filter-expression builders join same-facet selections into a single comma-separated field expression (unit-pinned in their own tests), and the range-slider component emits both bound-inclusion flags unconditionally. **Live:** anonymous catalog session on a chrome lane, real clicks throughout. **Note on the live axis — closed 2026-09-03:** the filter-apply then sort-persistence sequence has now been executed live on a lane that can click. Setting a non-default sort and then applying a facet filter, and separately performing a full filter reset, left the sort selection unchanged in both cases — the filter-apply path resets the page only, never the sort. Row 1 is therefore live-grounded rather than docs + source only.
...
|---------|-------------|-----------|--------|-------------|---------|--------|
| **Failure hidden in the response body, not in `errors[]`** | xAPI mutations **whose return type carries a payload-level error surface** (`addItem`, `addBulkItemsCart`) return HTTP 200 **and an EMPTY top-level GraphQL `errors[]`**, with the failure surfaced only inside the payload — a populated `validationErrors` on `CartType`/line items, or `Errors` on a bulk-add result. Checking HTTP status *or* the protocol-level `errors[]` misses it; the two are independent signals and a case must assert both. **`createOrderFromCart` does NOT share this shape** — its return type carries no per-field error surface, so a failure there is expected on the standard `errors[]` channel instead (established from that return type’s field list, not reproduced live — do not re-cite it as an example of this pattern without a fresh repro). A genuine unhandled backend exception is the opposite shape — non-empty `errors[]` **plus a null data object** (see ECL-14.9), a contrast established from source rather than forced live (never provoke a backend exception on a shared environment) | High | Test false-positive, missed bugs | BL-CART-001, BL-CART-002 | ECL-10.2 | [OBSERVED] |
| **Partial cart update** | The claim is that `changeCartItemQuantity` returns 200 while the `cart` response still carries the old quantity. **Not reproduced server-side** (audited 2026-09-03): the mutation’s own response AND an independent follow-up cart read both showed the new quantity, and the cart aggregate assigns the new quantity unconditionally before saving. Do not assert this as a backend response-shape defect without a fresh repro. A stale quantity seen in the storefront UI is a **client-cache** question, not this pattern — re-derive against frontend source before citing it | Medium | Wrong quantity in order | BL-CART-002 | ECL-2.1 | [OBSERVED] |
| **Missing or unpriced context args (`storeId`/`cultureName`/`currencyCode`)** | Nullability is **not uniform**, and that is the trap. `storeId` is non-null on the high-traffic catalog and cart queries, so omitting it *there* is a query-validation error rather than this pattern — but it is **nullable on many other operations**, and `cultureName`/`currencyCode` are nullable throughout. Omitting a nullable context arg returns HTTP 200 with a server-chosen default, so the result is wrong, empty or null with nothing signalling it. The sharpest live-reproducible case is an **explicit but unpriced** currency: a real product requested in a currency it has no price list for comes back 200, still counted in the total, with a **zero price** — neither an error nor an exclusion. Whether that zero is itself a defect is `BL-PRICE-005`’s question, not this row’s | Medium | Wrong product data in test | BL-PRICE-005 | ECL-12.1 | [OBSERVED] |
| **addItem with a master/parent product id** | Agent adds a product’s MASTER id (a product that declares variations) instead of one of its variation ids — `addItem` accepts it silently: empty `validationErrors`, and the cart carries a genuine line item bearing the master’s own SKU, indistinguishable from an ordinary single-SKU product. Nothing in the add-item command handler, the cart aggregate or the cart error describer checks for a variation-bearing product before accepting the id | Low-Medium | Wrong item ordered | — (no invariant governs this; proposed as a gap) | ECL-8.2 | [OBSERVED] |

**Amended:** 2026-09-03 (auto-applied, triangulated — ECL-AUDIT-2026-09-03; **first triangulation of this section**, which is the most-cited entry in the library). **Row 1** narrowed: the pattern holds for the mutations whose return type carries a payload-level error surface, `createOrderFromCart` is removed from that set (its return type has no such field, so its failures take the ordinary `errors[]` channel), the bulk mutation’s name is corrected to `addBulkItemsCart`, and `BL-ORD-001` is dropped from the BL cell — it governs order state-machine transitions, not an error shape. **Row 2** could not be reproduced server-side and now says so rather than asserting a defect: both the mutation response and an independent re-read returned the new quantity. **Row 3** rewritten around the real hazard: context-arg nullability is **not uniform** — non-null on the catalog and cart queries, nullable on many other operations and for `cultureName`/`currencyCode` throughout — so the row no longer implies a uniformly-omittable `storeId` in either direction; the live-reproducible case added is an explicit-but-unpriced currency returning a zero price with no error and no exclusion. **Row 4** CONFIRMED on all axes, but its `BL-CAT-006` citation was **wrong**: that invariant governs configurable-product configuration sections, a different mechanism from the master/variation product model this row describes. No invariant in the business-logic oracle covers master/variation add-to-cart behaviour, so the cell now declares a gap and the candidate is recorded in the proposals file. **Docs:** the validation-error object and the product master/variation model are documented; the payload-vs-`errors[]` mechanic and the silent-default behaviour are implementation detail no guide narrates (§1a class 1). **Source:** the cart module’s cart type, cart aggregate, add-item and change-quantity command handlers and error describer, plus the committed schema snapshot for argument nullability and the bulk mutation’s name. **Live:** anonymous-cart probes against the deployed environment. **Deliberately not forced live:** the unhandled-exception contrast (destructive on a shared environment) and `createOrderFromCart` (placing a real order for an audit probe) — both rest on source, and both say so above.
...
| **Company member role not applied** | New org member invited; role permissions (Buyer vs Manager vs Admin) are baked into the session token at mint time and are never live-pushed, so a role change does not take effect until the member's **next sign-in**. A plain page or session refresh rehydrates the cached token and is **not** sufficient — a test that reloads and expects the new role will read the old one. Documented behaviour, not a platform bug | Low-Medium | Feature access test incorrect | BL-B2B-005 | ECL-4.3 | [OBSERVED] |
...
**Amended:** 2026-09-03 (auto-applied, triangulated — ECL-AUDIT-2026-09-03; first triangulation of this section). **Row 3** was **DRIFT** on its trigger condition: corrected from "session refresh" to the member's **next sign-in**. The distinction is the whole point of the row — a tester who reloads the page and reads the old role concludes the platform is broken, when the reload deliberately reuses a cached token. **Row 5** CONFIRMED unchanged, which matters because it is the row that exists to PREVENT false bug reports (Frequency `High (false bug)`): the account sidebar still shows no standalone Addresses entry for a corporate account. **Rows 1, 2 and 4 were NOT confirmed and are NOT changed** — see `reports/ba/ecl-proposals-2026-09-03.md`. Row 1 (cart isolation loss) is **CONTRADICTORY**: source and live agree with each other and both disagree with the row — the org-switch path mints a fresh org-scoped token and forces an app-wide reload, and a cart established under one organization did not appear under another in either direction across two passes. Rows 2 and 4 are **UNGROUNDED**, each for a stated reason rather than a missing effort: the row-2 session could not be isolated from a residual prior organization selection, so the genuinely-first-login case was never exercised, and the two organizations available for row 4 held asymmetric address data (one had none), which cannot discriminate a cross-organization leak. **Docs:** the storefront account guide states a role change takes effect the next time the member signs in, and separately that the Addresses item appears only in personal accounts while the Corporate section appears only in corporate ones; the reset-on-switch mechanics of rows 1/2/4 are implementation detail no guide narrates (§1a class 1). **Source:** the storefront's user and auth composables — the organization-switch call that re-mints an organization-scoped token and broadcasts an app-wide reload, the token persisted in browser storage so a reload rehydrates rather than re-mints, and the corporate-member flag that gates personal-vs-corporate navigation. **Live:** an authenticated multi-organization session on an edge lane, real interaction throughout, cart left clean.
