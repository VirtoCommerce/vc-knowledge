...
**Two directions, both legitimate — don't mistake one for a contradiction.** `/qa-exploratory` and
`/qa-sbtm` read this file **subtractively**: a charter's value is novelty, so a known pattern is what
*not* to re-discover. `/qa-test` reads it **generatively**: a regression case's value is repeatability,
so a known way to break must *have* a case. Same file, opposite uses, because exploratory testing and
scripted testing earn their keep differently — see `/qa-exploratory` Step 5a for the same split applied
to the two shared oracles.

### How to maintain

- Add a new entry when a production or QA bug reveals a pattern likely to recur
- Each entry: pattern → past incident → detection probe → **archetype** → cross-ref
...
### Defect archetypes — the closed vocabulary

Every entry carries exactly one **`Archetype:`** token. An archetype is the *shape* of the failure —
orthogonal to the domain (Cart / Catalog / …) and to the root cause (`qa-defect`'s cause taxonomy).
It is what lets a test model ask *"which shapes has this surface not been probed for?"* instead of
*"is every acceptance criterion represented?"*.

**The tag is curated; every aggregate is derived.** A regex over bug prose cannot tell `SILENT` from
`PARITY` — it would silently manufacture a confident, wrong distribution, which is the
`measure-layout.ts` phantom-spacing-grid failure reproduced on a knowledge file. So write the token
by hand when you write the entry, and never transcribe a frequency into this file: archetype →
entry count, → bug-report count, → citing-case count are all computed at read time from this file +
`reports/bugs/**` + `regression/suites/**` (`.claude/rules/test-data.md` §GOLDEN RULE).

**Defect shapes** — these are scenario candidates:

| Token | The failure shape | Probe question |
|---|---|---|
| `SCOPE` | Cross-org / tenancy / permission-scope leak — a missing filter, caller-vs-queried confusion | Does org A's user reach org B's object, and does the *response body* leak any of B's fields? |
| `PARITY` | The same rule or value diverges across sibling surfaces — UI vs REST vs GraphQL, Admin vs storefront, bulk vs single, processor A vs B | Is the verdict identical on every path that reaches this rule? |
| `STALE` | Cache / index / projection / fixture serves superseded data | Read → write **via a different path** → read again: is it fresh? |
| `RACE` | Concurrency on a read-then-write invariant; ordering; double-submit | Fire N identical requests at once — does the invariant still hold? |
| `REPLAY` | Idempotency: a retried or duplicated effect applies twice | Same key + same payload → one effect; same key + different payload → error |
| `SILENT` | Success-shaped failure — `200` + `errors[]`, a wrong field name accepted, `itemsCount = 0`, a no-op that reports OK | Did anything actually change, or did it only *say* it did? |
| `MONEY` | Currency, culture, rounding, formatting, i18n | Does it hold in a non-USD currency and a non-`en` culture, and at the rounding midpoint? |
| `BOUNDARY` | Off-by-one, absent limit, null threshold, validation that checks the wrong property | What is rejected at exactly the edge, and one step past it? |
| `LIFECYCLE` | Illegal state transition accepted, or a legal one missing its cascade / side effect | Is the invalid transition rejected *explicitly*, not silently ignored? |
| `CONFIG` | Behaviour diverges on a flag, setting, or environment that the test never varied | Does this still hold with the flag off — and is "off" a clean absence or an error? |
| `FALLBACK` | A missing or wrong default — absent template, unrecognized enum treated as benign, null read as zero | What happens when the value is absent, unrecognized, or new? |
| `RENDER` | Visual / interaction affordance — contrast, layout shift, target size, keyboard reachability, responsive re-mount | Is it perceivable and operable at every viewport and by keyboard alone? |

**Non-defect entries** — these are false-positive guards, never scenario candidates:

| Token | Means |
|---|---|
| `BY-DESIGN` | The behaviour is intentional. Filing it is the bug. |
| `CONVENTION` | A fact about the environment, the API surface, the fixtures, or how to operate the tooling — not a product behaviour at all. |
