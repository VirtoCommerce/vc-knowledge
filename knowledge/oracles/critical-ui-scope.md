...
## Observed drift — matrix entries that no longer match the live build

> **Historical record.** The 048b case IDs cited below no longer exist (the suite was removed 2026-07-25). The *observations* about the live build remain valid and are the reason these four entries still need re-derivation before any replacement suite is authored.

Recorded 2026-07-25 from run **REG-2026-07-24-2121** (vcst-qa @ Theme 2.54.0-pr-2395-fd70). Each entry below was confirmed by a **live DOM check that completed** — the component exists on a reachable page but renders a different shape than the matrix assumes. These are **matrix drift, not product defects**: the affected 048b cases were triaged `STALE_TEST`, not `REAL_BUG`. The audit protocol / render-location rows above are left as-authored pending an `ui-ux-expert` re-derivation.
...
**Action for `ui-ux-expert`:** re-derive these four cells against the live build, then either update the audit protocols (§14, §16, §19, §20) and render-location rows to the new shapes, or — if a shape is a genuine regression rather than an intended redesign — escalate it as a product defect instead. Until then, treat the four cells as **unenforced**: `validate-critical-ui-scope.ts` still passes because the covering case IDs exist, but those cases cannot produce a meaningful verdict.

> Not recorded here (deliberately): 048b's remaining blocked cases traced to fixture drift, harness limits, or unattempted scope — not to matrix drift. See `reports/regression/REG-2026-07-24-2121/triage-report.md` §3.
...
5. Skeleton → table swap (slow 3G):
   ...
   run manually via /qa-design until a replacement suite exists
```
...
These are required for regression coverage of the configurable-product UX but are not gated by the validator (which only enforces the matrix). They lived in suite 048b and are **not currently executed by any suite** — run them via `/qa-design` until a replacement suite exists.
