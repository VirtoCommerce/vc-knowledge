...
sources:
  ...
  - knowledge/domain/b2b-organizations.md rev 1 (2026-09-09, amended 2026-09-11) — this file IS the previous rev; every D*/G* id carried forward
  ...
  - knowledge/api/graphql-schema.md (live introspection refreshed 2026-09-16 — newer than rev 1's 2026-09-09 snapshot; its TYPE sections are a curated allowlist per its own header, queries/mutations lists are complete)
  ...
  - knowledge/automation/storefront-config-flags.md (quote_statuses[] — 8-entry runtime dictionary)
  - knowledge/domain/sitemap.md:387-389 (xCatalog/xProfile/xCMS/Quote GraphQL module table) — found stale this pass, see §8
  - knowledge/oracles/business-logic.md — BL-B2B-003/004/005, BL-CROSS-006/008 read for orientation, cited not restated
excludes: Sales Rep (deliberate — a separate later pass, now its own domain map `knowledge/domain/sales-rep.md` rev 2; see §6 note)
---
...
| **Sales rep** | `/company/sales-reps` renders in the org sidebar; three rep routes are the only ones clearing inherited `requiresOrganization` | out of scope — now covered by `knowledge/domain/sales-rep.md` |
...
uses `quote`/`quotes` throughout; `knowledge/domain/sitemap.md:389` names `quoteRequest`/
...
`090`, `091`, `092b`, `093`, `097`) — now covered by `knowledge/domain/sales-rep.md`.
