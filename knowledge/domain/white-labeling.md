...
3. Edit **Theme preset name** → Add → paste name → select → **Save**
   - The list itself is `WhiteLabeling.ThemePresetNames`, a **platform-GLOBAL dictionary setting** (`objectId`/`objectType` null — `StoreLevelSettings` carries only `WhiteLabelingEnabled`), so every store and every org picks from the same values. vc-module-white-labeling ships exactly **one**, `Default`; everything else is a DB row somebody typed here. **Seeded** by `ensureThemePresetNames()` (`scripts/lib/seed-common.mjs`, runs inside `ensureStore` → `npm run seed:store`), which derives the names from the `theme_preset` columns of `test-data/stores/stores.csv` + `test-data/white-labeling/organizations.csv` and merges them in **additively** — `POST /api/platform/settings` rebuilds a dictionary's value set, so a blind write would delete operator-added presets. Logic + rationale: [`scripts/seed-data/store/theme-preset-specs.mjs`](../../../scripts/seed-data/store/theme-preset-specs.mjs).
   - The storefront resolves a stored name through `presetNameToFileName()` = `toLowerCase().replaceAll(" ", "-")`, so **`Purple Pink` and `purple-pink` render identically**, and a name it cannot resolve **falls back to `settings_data.json.current` silently** — a case asserting on that theme still PASSES. The resolvable set is generated, never guessed: [`scripts/lib/theme-presets.generated.json`](../../../scripts/lib/theme-presets.generated.json) (`npm run presets:sync`, drift-gated by `presets:check`). **Do not judge a preset by probing `/assets/presets/<name>.json`** — bundled presets are compiled into the JS and 404 as files; only a theme's *extra* presets are served that way. Getting this backwards on 2026-09-15 produced a confident, wrong "`red` is broken" (it is bundled, and is in the deployed vcst-qa bundle).
   - **Store-level record.** `stores.csv`'s `theme_preset` column is applied by `ensureStoreWhiteLabeling()` (same `ensureStore` path) to `POST`/`PUT /api/white-labeling`. It is a GET-merge-PUT: the live record also holds `logoUrl` / `footerLinkListName` / `mainMenuLinkListName`, which `stores.csv` cannot supply, and **`PUT` REPLACES the record** — a CSV-shaped body saves cleanly and blanks all three. `fieldsLostByWlWrite()` refuses that write. Only `themePresetName` ever moves; `isEnabled` is left alone on an existing record.
