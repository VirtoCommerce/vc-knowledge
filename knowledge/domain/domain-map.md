...
Fill-in shape for `knowledge/domain/<name>.md`. **Tier 2, on-demand** — cited from
...
**The reference implementation is `knowledge/domain/b2b-organizations.md`.** Read it before
...
amended: YYYY-MM-DD            # last incremental write-back (/qa-test 5-docs-map). Omit until one happens
stale_after_days: 60
...
sources:
  ...
  - knowledge/api/graphql-schema.md (introspection date) — when a contract layer is in scope
...
Written by `/qa-test` `5-docs-map`, one row per write-back, **append-only**. A map that only a refresh could
...
10. **An amendment (`/qa-test` `5-docs-map`) obeys 1–9 on the rows it touches, plus three of its own:** every
...
| `/qa-test` `5-docs-map` | **writes back what the run verified** — new §2 surfaces, a `D*` upgraded live, a `G*` `CLOSED`, a §4 count corrected — into §7, after the verdict. Costs no dispatch; touches `amended:` only |
