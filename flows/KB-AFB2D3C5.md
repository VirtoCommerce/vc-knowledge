...
- **Step An order CAN be deleted once placed, as well as cancelled. DELETE /api/order/customerOrders (operation OrderModule_DeleteOrdersByIds) removes orders by id, and the Admin orders list carries a Delete control both in the toolbar and in every row menu. Cancelling is the right choice when you want the document kept with an audit trail; deleting is available when you want the residue gone. Note that cancelling does NOT clear the shipment - see the entry on the cancellation cascade. Checked on platform 3.1007.27, Orders 3.1000.4.** — The step said an order cannot be deleted, only cancelled. That is false and was false when written: the derived plane has carried DELETE /api/order/customerOrders the whole time (KB-746B7535), and the Admin UI shows Delete in the toolbar and in every row menu. The written plane and the derived plane disagreed for twelve runs and nothing noticed, because kb validate checks that entries are well-formed and that indexes match their contents, never that a written claim survives the contract sitting beside it.  
  _observed vcptcore_stable, platform 3.1007.26 · 2026-09-15T13:44:38.481Z_
